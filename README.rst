...
* Part 1 (``inlist_colors``) evolves a 5 |Msun|, Z=0.014 metallicity model from the pre-main sequence, calculating synthetic photometry at each timestep using interpolated stellar atmosphere models. The test adds the following columns to the history file:
...
.. code-block:: console
   ...
   Grvs                 ! Gaia radial velocity spectrometer magnitude
...
* **Filter system**: Gaia photometric bands (Gbp_bright, Gbp, Gbp_faint, G, Grp, Grvs)
* **Reference spectrum**: Vega SED for magnitude zero-points
* **Distance**: 10 parsecs (for absolute magnitudes)
* **Initial parameters**: 5 M☉, Z = 0.014 metallicity
* **Evolution limits**: Maximum 100 models or 5 billion years
...
.. code-block:: console
   ...
   python static_SED_check.py   # Static SED analysis
   python batch_run.py          # Batch processing script
...
Last-Updated: 25Jul2025 by Niall Miller